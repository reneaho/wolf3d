...
void	render_column(int ray_nbr, double distance, t_mlx *mlx)
{
	...
	projected_slice_height = (double)SQUARE_SIZE / distance * mlx->player.dist_to_proj_plane;
...
{	 	
	int mx,my,index,side,ray_nbr;
	float vx,vy,rx,ry,ra,xo,yo,disV,disH,fish_eye_fix_beta; 

	mlx->player.ray_angle = mlx->player.pos_angle + (FOV / 2);
	fix_circle_angle(&mlx->player.ray_angle);
	for(ray_nbr=0;ray_nbr<1;ray_nbr++)
	{
		//---Vertical--- 
		index=0;
		disV=100000;
		float Tan = tan(ft_deg_to_rad(mlx->player.ray_angle));
		if (cos(ft_deg_to_rad(mlx->player.ray_angle)) > 0.001)
		{
			mlx->player.ray_x = (((int)mlx->player.pos_x >> 6) << 6) + 64;
			mlx->player.ray_y = (mlx->player.pos_x - mlx->player.ray_x) * Tan + mlx->player.pos_y;
			mlx->player.offset_x = 64;
			mlx->player.offset_y = -mlx->player.offset_x * Tan;
		}	//looking right
		else if (cos(ft_deg_to_rad(mlx->player.ray_angle)) < -0.001)
		{
			mlx->player.ray_x = (((int)mlx->player.pos_x >> 6) << 6) -0.0001;
			mlx->player.ray_y = (mlx->player.pos_x - mlx->player.ray_x) * Tan + mlx->player.pos_y;
			mlx->player.offset_x = -64;
			mlx->player.offset_y= -mlx->player.offset_x * Tan;
		}	//looking left
		...
		{ 
			...
			if (mlx->player.map_x >= 0 && mlx->player.map_x < mlx->map.width && \
				...
				mlx->map.matrix[mlx->player.map_y][mlx->player.map_x] == 1)
			{
				index = mlx->map.width;
				disV = cos(ft_deg_to_rad(mlx->player.ray_angle)) * (mlx->player.ray_x - mlx->player.pos_x) - sin(ft_deg_to_rad(mlx->player.ray_angle)) * (mlx->player.ray_y - mlx->player.pos_y);
			}	//hit         
			else
			{
		...
		disH = 100000;
		Tan = 1.0 / Tan; 
		if (sin(ft_deg_to_rad(mlx->player.ray_angle)) > 0.001)
		{
			mlx->player.ray_y = (((int)mlx->player.pos_y >> 6) << 6) -0.0001;
			mlx->player.ray_x = (mlx->player.pos_y - mlx->player.ray_y) * Tan + mlx->player.pos_x;
			mlx->player.offset_y= -64;
			mlx->player.offset_x= -mlx->player.offset_y * Tan;
		}	//looking up 
		else if (sin(ft_deg_to_rad(mlx->player.ray_angle)) < -0.001)
		{
			mlx->player.ray_y = (((int)mlx->player.pos_y >> 6) << 6) + 64;
			mlx->player.ray_x = (mlx->player.pos_y - mlx->player.ray_y) * Tan + mlx->player.pos_x;
			mlx->player.offset_y = 64;
			mlx->player.offset_x = -mlx->player.offset_y * Tan;
		}	//looking down
		...
		{ 
			...
			{
				index = mlx->map.height;
				disH = cos(ft_deg_to_rad(mlx->player.ray_angle)) * (mlx->player.ray_x - mlx->player.pos_x) - sin(ft_deg_to_rad(mlx->player.ray_angle)) * (mlx->player.ray_y - mlx->player.pos_y);
			}	//hit         
			else
			{
				mlx->player.ray_x += mlx->player.offset_x;
		...
		if (disV < disH)
		{
			...
			mlx->player.ray_y = vy;
			disH = disV;
			mlx->draw.color = WHITE;
			mlx->player.wall_texture_offset = (int)mlx->player.ver_coll_point_y % SQUARE_SIZE;
		...
		disH = disH * cos(ft_deg_to_rad(fish_eye_fix_beta));

		/* Render column if we are not on top of a wall */
		if (disH > 0)
			render_column(ray_nbr, disH, mlx);


		ray_nbr += 1;
		mlx->player.ray_angle -= mlx->player.degrees_per_ray;
		fix_circle_angle(&mlx->player.ray_angle);                                                        //go to next ray
	}
}//-----------------------------------------------------------------------------
